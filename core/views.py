...
def take_vitals(request, visit_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('core:doctor_assessment', visit_id=visit.id)  # Fixed: added 'core:'
        except Exception as e:
...
def doctor_assessment(request, visit_id):
    ...
    if request.method == 'POST':
        try:
            # Only use fields that exist in your DoctorQuestionnaire model
            chief_complaint = request.POST.get('chief_complaint')
            medical_history = request.POST.get('medical_history')
            current_medications = request.POST.get('current_medications')
            physical_exam_findings = request.POST.get('physical_exam_findings')
            
            # Create the doctor questionnaire with only the fields that exist in your model
...
def psychiatrist_assessment(request, visit_id):
    ...
    if request.method == 'POST':
        # Handle psychiatrist assessment form
        try:
            # Only use fields that exist in your PsychiatristQuestionnaire model
